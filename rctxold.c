...
int s = 0; // UDP socket for headtracker
unsigned int i, slen = sizeof(si_other) , recv_len;
char buf[BUFLEN];
...
int main (int argc, char *argv[]) {
	...
	while (done) {
		...
		if (recv_len > 0) {
			...
			printf("Data: %s\n" , buf);
			// Do something incredibly usefull with the data
		}
